...
Local Timelines: Verified via Search for specific evaluation windows (e.g., 60 days in CA vs 45 days in other states).
Local PTI Referrals: Direct links to state-mandated Parent Training and Information Centers.
Search Grounding Flow
User Query: "What is the timeline for an IEP evaluation in Texas?"
AI Action: Gemini executes a search query for "Texas TEA IEP evaluation timelines".
Synthesis: The model combines its internal training with search results.
Attribution: Grounding metadata is parsed to display "Verified Source" badges in the UI.
